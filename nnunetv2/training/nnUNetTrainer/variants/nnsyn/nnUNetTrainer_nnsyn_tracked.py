...
from nnunetv2.training.nnUNetTrainer.variants.nnsyn.nnUNetTrainer_nnsyn_loss_masked_anatomical_perception import nnUNetTrainer_nnsyn_loss_map
import torch
...
class nnUNetTrainer_nnsyn_track(nnUNetTrainer_nnsyn):
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        #         ct_plan_path = join(nnUNet_preprocessed, self.plans_manager.dataset_name, 'gt_plan', self.plans_manager.plans_name + '.json')
        ...
        gt_segmentation_path = None  # for final validation, do not use segmentation metrics
    ...
    def perform_actual_validation_epoch(self, save_probabilities: bool = False):
        ...
        # gt_segmentation_path = join(nnUNet_preprocessed, self.plans_manager.dataset_name, 'gt_target_segmentation_ts')
        # gt_segmentation_path = gt_segmentation_path if os.path.isdir(gt_segmentation_path) else None
        gt_segmentation_path = None  # for epoch validation, do not use segmentation metrics
        src_path = join(nnUNet_raw, self.plans_manager.dataset_name, 'imagesTr')
