...
if __name__ == "__main__":
    ...
    # important paths
    submission_name = f'task1_{datetime.datetime.now().strftime("%Y%m%d")}_3models_v2r_deformed_full' # need to rename
    ...
    config_AB = json.load(open(os.path.join(cur_file_path, "config_280__nnUNetTrainerMRCT__nnUNetPlans__3d_fullres.json")))
    process_region_dataset(config_AB, submission_path)

    config_HN = json.load(open(os.path.join(cur_file_path, "config_282__nnUNetTrainerMRCT__nnUNetPlans__3d_fullres.json")))
    process_region_dataset(config_HN, submission_path)

    config_TH = json.load(open(os.path.join(cur_file_path, "config_284__nnUNetTrainerMRCT__nnUNetPlans__3d_fullres.json")))
