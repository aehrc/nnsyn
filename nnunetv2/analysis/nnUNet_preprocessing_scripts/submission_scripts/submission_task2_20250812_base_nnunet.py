...
def prepare_validation_dataset(data_root, dataset_name, task=1):
    ...
    list_data_input = sorted(glob.glob(input_img_path, recursive=True))
    list_data_mask = sorted(glob.glob(os.path.join(data_root, region, '**','mask.mha'), recursive=True))
    print("input1 ---", len(list_data_input), list_data_input)
    print("input2 ---", len(list_data_mask), list_data_mask)
    

    # copy validation images to imagesVal
    with ThreadPoolExecutor() as executor:
        list(tqdm(executor.map(lambda data_path: _process_file(data_path, dataset_data_path, "_0000"), list_data_input), total=len(list_data_input)))
...
def predict_and_revert(dataset_name, method_folder_name, submission_path, fold=0):
    '''
    ...
    os.system(f"nnUNetv2_predict -d {dataset_name} -i {input_dir} -o {output_dir} -c {configuration} -p {plan_name} -tr {trainer} -f {fold}")
...
def export_checkpoints(dataset_name, method_folder_name, submission_path, fold=0):
    ...
    """
    trainer, plan_name, configuration = method_folder_name.split('__')
    output_dir = os.path.join(submission_path, submission_name, "checkpoints")
    save_path = os.path.join(output_dir, f'{dataset_name.split("_")[0]}_{method_folder_name}_fold{fold}.zip')
    ...
    # move ct_plan to the save_path
    
    ct_plan_path = os.path.join(os.environ["nnUNet_preprocessed"], dataset_name, f"gt_{plan_name}.json")
    shutil.copyfile(ct_plan_path, os.path.join(output_dir, f'{dataset_name.split("_")[0]}_gt_{plan_name}.json'))

    # move one predicted image per checkpoint
    predicted_image = sorted(glob.glob(os.path.join(os.path.join(submission_path, submission_name, dataset_name) + "_revert_norm", "*.mha")))[0]
    new_name = os.path.join(output_dir, os.path.basename(predicted_image))
    shutil.move(predicted_image, new_name)
    print(f"Moved {predicted_image} to {new_name}")
...
if __name__ == "__main__":
    ...
    # important paths
    TASK = 2
    DESCRIPTION = "base_nnunet"
    DATASET_NAME_AB = "Dataset580_synthrad2025_task2_CBCT_AB_pre_v2r_stitched"
    DATASET_NAME_HN = "Dataset582_synthrad2025_task2_CBCT_HN_pre_v2r_stitched"
    DATASET_NAME_TH = "Dataset584_synthrad2025_task2_CBCT_TH_pre_v2r_stitched"
    METHOD_FOLDER_AB = "nnUNetTrainerMRCT_track__nnUNetPlans__3d_fullres"
    METHOD_FOLDER_HN = "nnUNetTrainerMRCT_track__nnUNetPlans__3d_fullres"
    METHOD_FOLDER_TH = "nnUNetTrainerMRCT_track__nnUNetPlans__3d_fullres"
    FOLD_AB = 0
    FOLD_HN = 0
    FOLD_TH = 0
