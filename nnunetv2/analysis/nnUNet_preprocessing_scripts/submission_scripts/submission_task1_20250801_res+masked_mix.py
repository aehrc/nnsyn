...
def process_mask_file(data_path, dataset_path):
    filename = data_path.split(os.sep)[-2]
...
def process_region_dataset(config, submission_path):
    ...
    list_data_mask = sorted(glob.glob(os.path.join(data_root, dataset_shortname, '**','mask.mha'), recursive=True))
    ...
    with ThreadPoolExecutor() as executor:
        list(tqdm(executor.map(lambda data_path: process_mask_file(data_path, dataset_data_path), list_data_mask), total=len(list_data_mask)))

    # predict images
    input_dir = os.path.join(dataset_data_path, 'imagesVal')
    mask_dir = os.path.join(dataset_data_path, 'mask')
    ...
    ct_plan_path = f"/datasets/work/hb-synthrad2023/work/synthrad2025/bw_workplace/data/nnunet_struct/preprocessed/Dataset{config['dataset_id'] + 1}_{config['dataset_target_name']}/{config['plan']}.json"
...
if __name__ == "__main__":
    ...
    # important paths
    submission_name = f'task1_{datetime.datetime.now().strftime("%Y%m%d")}_3models_v2r_deformed_full_res+masked+mix' # need to rename
    ...
    config_AB = json.load(open("/datasets/work/hb-synthrad2023/work/synthrad2025/bw_workplace/ref/nnUNet_translation/nnunetv2/analysis/nnUNet_preprocessing_scripts/Dataset290-AB-norm-V2R-resnet/config_290__nnUNetTrainerMRCT__nnUNetResEncUNetLPlans__3d_fullres.json"))
    process_region_dataset(config_AB, submission_path)

    config_HN = json.load(open("/datasets/work/hb-synthrad2023/work/synthrad2025/bw_workplace/ref/nnUNet_translation/nnunetv2/analysis/nnUNet_preprocessing_scripts/Dataset290-AB-norm-V2R-resnet/config_292__nnUNetTrainerMRCT__nnUNetResEncUNetLPlans__3d_fullres.json"))
    process_region_dataset(config_HN, submission_path)

    config_TH = json.load(open("/datasets/work/hb-synthrad2023/work/synthrad2025/bw_workplace/ref/nnUNet_translation/nnunetv2/analysis/nnUNet_preprocessing_scripts/Dataset270-AB-norm-V2R-mask/config_274__nnUNetTrainerMRCT__nnUNetPlans__3d_fullres.json"))
