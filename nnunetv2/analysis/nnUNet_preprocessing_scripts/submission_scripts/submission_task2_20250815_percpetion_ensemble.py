...
def predict_and_revert(dataset_name, method_folder_name, submission_path, fold=0):
    ...
    print(f"nnUNetv2_predict -d {dataset_name} -i {input_dir} -o {output_dir} -c {configuration} -p {plan_name} -tr {trainer} -f {fold}")
...
def export_checkpoints(dataset_name, method_folder_name, submission_path, fold=0):
    ...
    """
    trainer, plan_name, configuration = method_folder_name.split('__')
    fold_name = f'fold_{fold}' if isinstance(fold, int) else f'fold_{fold.replace(" ", "_")}'
    save_path = os.path.join(submission_path, submission_name, dataset_name + "_revert_norm", f'{dataset_name.split("_")[0]}_{method_folder_name}_{fold_name}.zip')
    print(f"Exporting checkpoints to {save_path}...")
...
if __name__ == "__main__":
    ...
    # important paths
    TASK = 2
    DESCRIPTION = "perception_masked_ensemble_fold0_postchallenge_testing"
    DATASET_NAME_AB = "Dataset540_synthrad2025_task2_CBCT_AB_pre_v2r_stitched_masked_both"
    DATASET_NAME_HN = "Dataset542_synthrad2025_task2_CBCT_HN_pre_v2r_stitched_masked_both"
    DATASET_NAME_TH = "Dataset544_synthrad2025_task2_CBCT_TH_pre_v2r_stitched_masked_both"
