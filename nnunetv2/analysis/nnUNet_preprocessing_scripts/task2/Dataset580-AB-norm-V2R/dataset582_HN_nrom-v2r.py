...
if __name__ == '__main__':
    ...
    REGION = "HN"
    config = {
        "dataset_id": 582,  # Updated to 200 for CT noNorm
        "dataset_data_name": f"synthrad2025_task{TASK}_CBCT_{REGION}_pre_v2r_stitched",
        "dataset_target_name": f"synthrad2025_task{TASK}_CT_{REGION}_pre_v2r_stitched",
        ...
        "preprocessing_CBCT": "CT_zscore_synthrad",
    ...
    list_data_cbct = sorted(glob.glob(os.path.join(config["data_root"], '**','cbct.mha'), recursive=True))
    ...
    print("input1 ---", len(list_data_cbct), list_data_cbct[:3])
    ...
    create_dataset_json(config, config["preprocessing_CBCT"], dataset_data_path)
    ...
    move_preprocessed(nnunet_datas_preprocessed_dir, nnunet_targets_preprocessed_dir, "nnUNetPlans_3d_fullres")
